-- PHASE 5: KINSHIP PATH FINDER
-- Finds the shortest understandable relationship path between two approved people.
...
as $$
  ...
  graph as (
    ...
    select r.source_person_id, r.target_person_id, 'sibling'::text, false
    from approved_relationships r
    where r.relation_type = 'sibling'

    union

    select r.target_person_id, r.source_person_id, 'sibling'::text, false
    from approved_relationships r
    where r.relation_type = 'sibling'

    union

    select s.from_id, s.to_id, 'sibling'::text, true
    from inferred_sibling_edges s
  ),
  walk as (
    select
      0::integer as depth,
      p_from_person_id as current_id,
      array[p_from_person_id]::uuid[] as path_ids,
    ...
    where w.depth < least(greatest(coalesce(p_max_depth, 6), 1), 10)
