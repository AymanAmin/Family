...
app = replace_once(app,"import PeoplePicker from './components/PeoplePicker'","import PeoplePicker from './components/PeoplePicker'\nimport EventPeopleFields, { eventParticipantPayload, eventParticipantsRequired } from './components/EventPeopleFields'\nimport { PersonVerifiedBadge } from './components/VerifiedBadge'",'feature imports')
app = replace_once(app,"const MySubmissionActivity = lazy(() => import('./components/MySubmissionActivity'))","const MySubmissionActivity = lazy(() => import('./components/MySubmissionActivity'))\nconst DirectRelationshipEditor = lazy(() => import('./components/DirectRelationshipEditor'))",'relationship editor lazy import')
app = replace_once(app,"  const [relationships, setRelationships] = useState<PersonRelationship[]>([])\n  const [relationsLoading, setRelationsLoading] = useState(false)","  const [relationships, setRelationships] = useState<PersonRelationship[]>([])\n  const [relationsLoading, setRelationsLoading] = useState(false)\n  const [relationshipRefresh, setRelationshipRefresh] = useState(0)",'relationship refresh state')
app = replace_once(app,"  const [eventForm, setEventForm] = useState({ event_type: 'general', title: '', family_id: '', event_date: '', location_name: '', description: '' })","  const [eventForm, setEventForm] = useState({ event_type: 'general', title: '', family_id: '', event_date: '', location_name: '', description: '' })\n  const [eventPeopleForm, setEventPeopleForm] = useState({ primary_person_id: '', secondary_person_id: '' })",'event people state')
...
app=replace_once(app,'              <button className="ghost-button" onClick={signOut} disabled={busy}>خروج</button>','', 'remove header logout')
app=replace_once(app,'              <div><span className="eyebrow">ملف شخص</span><h1>{selectedPerson.full_name}</h1><p>{selectedPerson.description || \'لا توجد نبذة مضافة لهذا الشخص.\'}</p></div>','              <div><span className="eyebrow">ملف شخص</span><div className="person-title-line"><h1>{selectedPerson.full_name}</h1><PersonVerifiedBadge personId={selectedPerson.id} /></div><p>{selectedPerson.description || \'لا توجد نبذة مضافة لهذا الشخص.\'}</p></div>','person verified badge')
app=replace_once(app,'            <PersonFamilyMemberships personId={selectedPerson.id} sessionUserId={session?.user.id} isAdmin={isAdmin} onChanged={loadCommunityData} />','            <PersonFamilyMemberships personId={selectedPerson.id} recordCreatedBy={selectedPerson.created_by} sessionUserId={session?.user.id} isAdmin={isAdmin} onChanged={async () => { await loadCommunityData(); await openPersonById(selectedPerson.id) }} />\n            <Suspense fallback={<LazyPanelFallback />}><DirectRelationshipEditor personId={selectedPerson.id} sessionUserId={session?.user.id} isAdmin={isAdmin} onChanged={() => setRelationshipRefresh((value) => value + 1)} /></Suspense>','primary family and relationship edit UI')
app=replace_once(app,'              <KinshipNetwork\n                personId={selectedPerson.id}','              <KinshipNetwork\n                key={`${selectedPerson.id}-${relationshipRefresh}`}\n                personId={selectedPerson.id}','kinship refresh key')
...
if start==-1 or end==-1:
    if 'event-people-fields' not in app: raise RuntimeError('Event form block not found')
else:
...
if "import './social-verification-events.css'" not in main: main=main.replace("import './admin-death.css'","import './admin-death.css'\nimport './social-verification-events.css'")
app_path.write_text(app,encoding='utf-8'); main_path.write_text(main,encoding='utf-8')
print('Applied six requested person/event features to the app.')
# retry after directory typing fixes
